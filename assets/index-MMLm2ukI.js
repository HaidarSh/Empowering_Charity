...
import { bD as buffer, bE as v4, bF as hexlify, t as toUtf8Bytes, $ as AmountSchema, bz as BigNumberSchema, bC as BigNumberishSchema, b2 as MaxUint256, bg as parseUnits, bG as SnapshotSchema, c as __vitePreload, a1 as ContractWrapper, B as BigNumber, b8 as hexZeroPad, aR as AddressZero, aM as normalizePriceValue, b7 as isNativeToken, bf as fetchCurrencyValue, aF as formatUnits, Z as z, a0 as AddressOrEnsSchema, bH as AddressSchema, aN as NATIVE_TOKEN_ADDRESS, by as BasisPointsSchema, aJ as keccak256, b9 as fetchCurrencyMetadata, bI as keccak256$1, bJ as SnapshotEntryWithProofSchema, bK as QuantitySchema, bL as BytesLikeSchema, bM as SnapshotInputSchema, bN as DuplicateLeafsError, bO as getDefaultExportFromCjs } from "./index-R4o_uq0C.js";
import { r as reverse, t as treeifyExports } from "./treeify-BRWJLCWc.js";
import { S as StartDateSchema, E as EndDateSchema } from "./assertEnabled-d1700f0b.browser.esm-DFK6vCrZ.js";
import { N as NFTInputOrUriSchema } from "./setErc20Allowance-7f76f677.browser.esm-LDt0ehVI.js";
class Base {
  /**
   * print
  ...
  bufferIndexOf(array, element) {
    ...
    if (isSorted) {
      return this.binarySearch(array, element, buffer.Buffer.compare);
    }
  ...
  static bufferify(value) {
    if (!buffer.Buffer.isBuffer(value)) {
      if (typeof value === "object" && value.words) {
        return buffer.Buffer.from(value.toString(convertWordsToBuffer), "hex");
      } else if (Base.isHexString(value)) {
        return buffer.Buffer.from(value.replace(/^0x/, ""), "hex");
      } else if (typeof value === "string") {
        return buffer.Buffer.from(value);
      } else if (typeof value === "bigint") {
        return buffer.Buffer.from(value.toString(16), "hex");
      } else if (value instanceof Uint8Array) {
        return buffer.Buffer.from(value.buffer);
        ...
        return buffer.Buffer.from(value.buffer, value.byteOffset, value.byteLength);
      }
    }
    return value;
  }
  ...
  static bufferToHex(value) {
    let withPrefix = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : true;
    return `${withPrefix ? "0x" : ""}${(value || buffer.Buffer.alloc(0)).toString("hex")}`;
  }
  /**
   * bufferify
   * @desc Returns a buffer type for the given value.
  ...
  bufferifyFn(f) {
    return (value) => {
      ...
      if (this.isHexString(v)) {
        return buffer.Buffer.from(v.replace("0x", ""), "hex");
      }
      if (typeof v === "string") {
      ...
      if (ArrayBuffer.isView(v)) {
        return buffer.Buffer.from(v.buffer, v.byteOffset, v.byteLength);
      }
      ...
      return buffer.Buffer.from(hexResult, "hex");
    };
  }
  /**
   * isHexString
...
function hexStringToArrayBuffer(hexString) {
  const buffer2 = new Uint8Array(hexString.length / 2);
  for (let i = 0; i < hexString.length; i += 2) {
    buffer2[i / 2] = parseInt(hexString.substring(i, i + 2), 16);
  }
...
class MerkleTree extends Base$1 {
  ...
  constructor(leaves, hashFn) {
    ...
    __publicField(this, "concatenator", buffer.Buffer.concat);
    ...
    if (options.fillDefaultHash) {
      ...
      } else if (buffer.Buffer.isBuffer(options.fillDefaultHash) || typeof options.fillDefaultHash === "string") {
  ...
  processLeaves(leaves) {
    ...
    if (this.sortLeaves) {
      this.leaves = this.leaves.sort(buffer.Buffer.compare);
    }
  ...
  createHashes(nodes) {
    ...
    while (nodes.length > 1) {
      ...
      for (let i = 0; i < nodes.length; i += 2) {
        if (i >= layerLimit) {
          ...
          if (nodes.length % 2 === 1) {
            ...
            if (this.isBitcoinTree) {
              hash2 = this.hashFn(this.concatenator([reverse(data), reverse(data)]));
              hash2 = reverse(this.hashFn(hash2));
        ...
        if (this.sortPairs) {
          combined.sort(buffer.Buffer.compare);
        }
  ...
  getLeaves(values) {
    if (Array.isArray(values)) {
      if (this.hashLeaves) {
        ...
        if (this.sortLeaves) {
          values = values.sort(buffer.Buffer.compare);
        }
      }
  ...
  getRoot() {
    ...
    return this.layers[this.layers.length - 1][0] || buffer.Buffer.from([]);
  }
  /**
   * getHexRoot
  ...
  getProof(leaf, index) {
    ...
    if (!Number.isInteger(index)) {
      ...
      for (let i = 0; i < this.leaves.length; i++) {
        if (buffer.Buffer.compare(leaf, this.leaves[i]) === 0) {
          index = i;
        }
      }
    }
  ...
  getMultiProof(tree, indices) {
    ...
    if (!indices.every(Number.isInteger)) {
      let els = indices;
      if (this.sortPairs) {
        els = els.sort(buffer.Buffer.compare);
      }
  ...
  verify(proof, targetNode, root) {
    ...
    for (let i = 0; i < proof.length; i++) {
      ...
      } else if (buffer.Buffer.isBuffer(node)) {
        data = node;
        isLeftNode = true;
      ...
      if (this.isBitcoinTree) {
        ...
        if (this.sortPairs) {
          if (buffer.Buffer.compare(hash, data) === -1) {
    ...
    return buffer.Buffer.compare(hash, root) === 0;
  }
  /**
   * verifyMultiProof
  ...
  try {
    if (!activeClaimCondition.merkleRootHash.toString().startsWith(AddressZero)) {
      ...
      if (snapshotEntry) {
        ...
        priceInProof = snapshotEntry.price === void 0 || snapshotEntry.price === "unlimited" ? MaxUint256 : await normalizePriceValue(contractWrapper.getProvider(), snapshotEntry.price, snapshotEntry.currencyAddress || AddressZero);
...
function resolveOrGenerateId(requestUId) {
  if (requestUId === void 0) {
    ...
    v4({}, buffer2);
    return hexlify(toUtf8Bytes(buffer2.toString("hex")));
  } else {
...
export {
  BaseSignaturePayloadInput as B,
  ClaimEligibility as C,
  MintRequest1155 as M,
  Signature1155PayloadInputWithTokenId as S,
  approveErc20Allowance as a,
  Signature1155PayloadOutput as b,
  convertQuantityToBigNumber as c,
  deepEqual as d,
  abstractContractModelToLegacy as e,
  fetchSnapshotEntryForAddress as f,
  abstractContractModelToNew as g,
  prepareClaim as h,
  SnapshotFormatVersion as i,
  Signature20PayloadInput as j,
  Signature20PayloadOutput as k,
  legacyContractModelToAbstract as l,
  MintRequest20 as m,
  newContractModelToAbstract as n,
  Signature721WithQuantityInput as o,
  processClaimConditionInputs as p,
  Signature721WithQuantityOutput as q,
  resolveOrGenerateId as r,
  MintRequest721 as s,
  transformResultToClaimCondition as t,
  updateExistingClaimConditions as u,
  MintRequest721withQuantity as v
};
